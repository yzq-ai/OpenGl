...
namespace test
{
        ...
        m_Translation(glm::vec3(100, 0, 0))
    {
        ...
        m_IndexBuffer = std::make_unique<IndexBuffer>(indices, 12);//索引缓冲区

        m_Shader = std::make_unique<Shader>("res/shaders/Batch.shader");//使用批处理着色器
        m_Shader->Bind();

        m_Texture[0] = std::make_unique<Texture>("res/textures/aveter.png");//插槽绑定纹理
        m_Texture[1] = std::make_unique<Texture>("res/textures/avater.png");//插槽绑定纹理

        for (size_t i = 0; i < 2; i++)
        {
            m_Texture[i]->Bind(i);
        }
        int samplers[2] = { 0, 1 };//纹理插槽0 和 1
